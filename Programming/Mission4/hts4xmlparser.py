...
# Gets the File
br.retrieve('https://www.hackthissite.org/missions/prog/4/XML',
            'plotMe.xml.bz2')
    ...
    while not done:
        ...
        for event in pygame.event.get():  # User did something
            ...
            for coord in line_blue_x_start:
                pygame.draw.line(screen, BLUE, [line_blue_x_end[i], line_blue_y_end[i]], [
                         line_blue_x_start[i], line_blue_y_start[i]], 1)
                i += 1
                i = 0
            for coord in arc_blue_start:
                rect = (arc_blue_x_center[i]-arc_blue_radius[i], arc_blue_y_center[i]
                        - arc_blue_radius[i], arc_blue_radius[i]*2, arc_blue_radius[i]*2)
                pygame.draw.arc(screen, BLUE, rect, degreesToRadians(
                    (arc_blue_extend[i])), degreesToRadians(arc_blue_extend[i]+arc_blue_start[i]), 1)
                i += 1
                i = 0
            for coord in line_green_x_start:
                pygame.draw.line(screen, GREEN, [line_green_x_start[i], line_green_y_start[i]], [
                             line_green_x_end[i], line_green_y_end[i]], 1)
                i += 1
                i = 0
            for coord in arc_green_start:
                rect = (arc_green_x_center[i]-arc_green_radius[i], arc_green_y_center[i]
                        - arc_green_radius[i], arc_green_radius[i]*2, arc_green_radius[i]*2)
                pygame.draw.arc(screen, GREEN, rect, degreesToRadians(
                    (arc_green_extend[i])), degreesToRadians(arc_green_extend[i]+arc_green_start[i]), 1)
                i += 1
                i = 0
            for coord in line_red_x_start:
                pygame.draw.line(screen, RED, [line_red_x_start[i], line_red_y_start[i]], [
                             line_red_x_end[i], line_red_y_end[i]], 1)
                i += 1
                i = 0
            for coord in arc_red_start:
                rect = (arc_red_x_center[i]-arc_red_radius[i], arc_red_y_center[i]
                        - arc_red_radius[i], arc_red_radius[i]*2, arc_red_radius[i]*2)
                pygame.draw.arc(screen, RED, rect, degreesToRadians(
                    (arc_red_extend[i])), degreesToRadians(arc_red_extend[i]+arc_red_start[i]), 1)
                i += 1
                i = 0
            for coord in line_yellow_x_start:
                pygame.draw.line(screen, YELLOW, [line_yellow_x_start[i], line_yellow_y_start[i]], [
                             line_yellow_x_end[i], line_yellow_y_end[i]], 1)
                i += 1
                i = 0
            for coord in arc_yellow_start:
                rect = (arc_yellow_x_center[i]-arc_yellow_radius[i], arc_yellow_y_center[i]
                        - arc_yellow_radius[i], arc_yellow_radius[i]*2, arc_yellow_radius[i]*2)
                pygame.draw.arc(screen, YELLOW, rect, degreesToRadians(
                    (arc_yellow_extend[i])), degreesToRadians(arc_yellow_extend[i]+arc_yellow_start[i]), 1)
                i += 1
                i = 0
            for coord in line_white_x_start:
                pygame.draw.line(screen, WHITE, [line_white_x_start[i], line_white_y_start[i]], [
                             line_white_x_end[i], line_white_y_end[i]], 1)
                i += 1
                i = 0
            for coord in arc_white_start:
                rect = (arc_white_x_center[i]-arc_white_radius[i], arc_white_y_center[i]
                        - arc_white_radius[i], arc_white_radius[i]*2, arc_white_radius[i]*2)
                pygame.draw.arc(screen, WHITE, rect, degreesToRadians(
                    (arc_white_extend[i])), degreesToRadians(arc_white_extend[i]+arc_white_start[i]), 1)
                i += 1
                i = 0
        ...
        br.select_form(nr=0)
        br.form['solution'] = input("BlueGreenRedYellowWhite: ")
        br.submit()
