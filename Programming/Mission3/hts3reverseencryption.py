...
def Total(str_guess, passhash_guess, previous, times_run):
	if(times_run == 0):
		intTotal = ord(str(str_guess)[0:1]) - \
		               int(elist[times_run]) + int(passhash_guess, 16)
		return intTotal
	else:
	...
	for j in range(len(hex_chars)):
		for i in range(len(characters)):
			if(((ord(characters[i]) + int(hex_chars[j], 16) - Total(characters[i], hex_chars[j], 0, 0)) == int(elist[0]))):
				intTotal = Total(characters[i], hex_chars[j], Total(
					characters[i], hex_chars[j], 0, 0), 1)
...
def HashCalculate(char_guess, depth, hash_string, total):
	if(depth > 99):
		...
		if(SpecialChar(depth) != "0"):
			intTotal = Total(char_guess+SpecialChar(depth),
			                 hash_string[0:1], total, depth+1)
			strDone = HashCalculate(char_guess+SpecialChar(depth),
			                        depth+1, hash_string, intTotal)
			return strDone
		else:
			for i in range(len(characters)):
				if((ord(characters[i]) + int(hash_string[(depth % 32):(depth % 32+1)], 16) - total) == int(elist[depth])):
					intTotal = Total(
						char_guess+characters[i], hash_string[0:1], total, depth+1)
					strDone = HashCalculate(
						char_guess+characters[i], depth+1, hash_string, intTotal)
					if(strDone != "0"):
						return strDone
		return "0"
			...
			if(SpecialChar(depth) != "0"):
				if((ord(SpecialChar(depth)) + int(hex_chars[j], 16) - total) == int(elist[depth])):
					intTotal = Total(char_guess+SpecialChar(depth),
					                 hash_string[0:1], total, depth+1)
					strDone = HashCalculate(char_guess+SpecialChar(depth),
					                        depth+1, hash_string+hex_chars[j], intTotal)
					if(strDone != "0"):
						return strDone
			else:
				for i in range(len(characters)):
					if((ord(characters[i]) + int(hex_chars[j], 16) - total) == int(elist[depth])):
						intTotal = Total(
							char_guess+characters[i], hash_string[0:1], total, depth+1)
						strDone = HashCalculate(
							char_guess+characters[i], depth+1, hash_string+hex_chars[j], intTotal)
						if(strDone != "0"):
							return strDone
